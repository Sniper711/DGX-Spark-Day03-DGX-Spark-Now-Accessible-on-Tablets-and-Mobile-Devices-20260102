...
# DGX Spark (第03天) 把 DGX Spark 擴展到 安卓 蘋果 平板上 20260101
## 🟩 中文版
> ## 適用情境 與 優點
...
- [在平板安裝並設定 WireGuard APP：](#在平板安裝並設定-wireguard-app)
...
- [在平板安裝並設定 Termius APP：](#在平板安裝並設定-termius-app)
...
- [在平板 依序啟動 WireGuard 與 Termius 服務，再 瀏覽 Ollama 服務網址：](#在平板-依序啟動-wireguard-與-termius-服務再-瀏覽-ollama-服務網址)
...
### Step 3. 測試安卓/蘋果平板的 WireGuard Client 連線：
在 DGX Spark 的 WireGuard Server 已開機情形下
- Step 3-1. 在平板 WireGuard APP找到剛設定好的連線 (我的範例是**`DGXSparkVPN`**)，按下去。
- Step 3-2. 若VPN建立成功。平板 WireGuard APP 內會開始顯示已連線時長的讀秒，且平板右上角會出現一隻鑰匙圖案。

--- 

## 在平板安裝並設定 `Termius` APP：
...
## 在平板 依序啟動 `WireGuard` 與 `Termius` 服務，再 瀏覽 `Ollama 服務網址`：
